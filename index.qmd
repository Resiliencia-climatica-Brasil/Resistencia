...
cobertura do solo fornecida pelo MapBiomas coleção 8.0 para o ano de
2021 [@mapbiomas_project_collection_2020]. Complementamos essa camada do uso do solo com informações sobre: 
::: {}1.Tamanho dos rios, 2.presença de infraestruturas de transporte e 3.infraestruturas de energia. :::
...
As informações sobre infraestrutura de transporte vieram das camadas de estradas pavimentadas, estradas não-pavimentadas e ferrovias fornecidas pelo Instituto Brasileiro de Geografia e Estatística (IBGE) \[BCIM250, 2021\].<!-- seria legal o link da base do IBGE e linhas de transmissão de energia  aqui -->
...
de reamostragem foi realizado pelo seguinte caminho `ToolsBox > Raster > ...`. Em `Resample`, entramos o *raster* original do MapBiomas em `Input Raster`, atribuímos um nome para `Output Raster Dataset` e a nova resolução em `Output Cell Size (optional)` (0.00083333333). A resolução foi calculada… Escolhemos o método `NEAREST` em `Resampling technique (optional)` (@fig-1).
...
A classe do MapBiomas de cobertura do solo na escala de 90m que é equivalente a águas abertas (rios, lagos e oceanos) foi subdividida em quatro classes distintas de acordo com a largura dos corpos d’água já que rios mais largos agem como bareiras mais fortes à movimentação das espécies. Para fazer a divisao, extraímos a máscara de corpos d’água e dividimos essa classe única em pedaços menores, regionalizando suas larguras por trechos. Esta divisão ocorreu com o uso da ferramenta “Raster Calculator” (@fig-2), por meio da intersecção entre as camadas matriciais de águas abertas e de bacias hidrográficas no nível 8, produzidas por **Lehner &Grill (2013)**.
...
![Superfície de resistência gerada a partir do mapa com 37 classes de cobertura do solo - resultante da junção das superfícies do MapBiomas, de infraestrutura e de largura dos rios.](figs/fig6){width=100% #fig-6}
